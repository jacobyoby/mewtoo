...
class MemoryAddresses:
    ...
    INVENTORY_START = (
        0xD31E  # Inventory items (each item is 2 bytes: item_id, quantity)
    )
...
class MemoryReader:
    ...
    def read_byte(self, address: int) -> int:
        ...
        try:
            ...
            if hasattr(self.pyboy, "memory") and (
                isinstance(self.pyboy.memory, (list, tuple))
                or hasattr(self.pyboy.memory, "__getitem__")
            ):
    ...
    def read_pokemon_party(self) -> list[dict]:
        ...
        for i in range(party_count):
            pokemon_addr = MemoryAddresses.PARTY_POKEMON_START + (
                i * MemoryAddresses.POKEMON_DATA_SIZE
            )

            species = self.read_byte(pokemon_addr + MemoryAddresses.POKEMON_SPECIES)
            hp_current = self.read_word(
                pokemon_addr + MemoryAddresses.POKEMON_HP_CURRENT
            )
    ...
    def read_health_hp(self) -> dict[str, any]:
        ...
        return {
            ...
            "total_hp_percent": (total_hp / total_max_hp * 100)
            if total_max_hp > 0
            else 0,
    ...
    def read_battle_state(self) -> dict[str, any]:
        ...
        battle_info = {
            ...
            if battle_type == 0
            else ("wild" if battle_type == 1 else "trainer"),
            "wild_pokemon_species": wild_species
            if in_battle and battle_type == 1
            else 0,
        }
