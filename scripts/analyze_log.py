...
def analyze_log(log_path: str):
    ...
    if steps_log:
        ...
        print(
            f"Steps with Screen Text: {len(steps_with_text)}/{len(steps_log)} ({len(steps_with_text) / len(steps_log) * 100:.1f}%)"
        )
        if steps_with_text:
            print("\nSample Screen Texts:")
        ...
        if len(steps_log) > 1:
            ...
            print(
                f"Average Frames per Step: {sum(frame_counts) / len(frame_counts):.1f}"
            )
