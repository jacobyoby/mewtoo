...
def analyze_screenshot_detailed(image_path: str):
    ...
    if len(arr.shape) == 3:
        ...
        print(
            f"White pixels (>240): {white_pixels}/{total_pixels} ({white_percentage:.1f}%)"
        )
        ...
        print(
            f"Black pixels (<15): {black_pixels}/{total_pixels} ({black_percentage:.1f}%)"
        )
...
if __name__ == "__main__":
    if len(sys.argv) > 1:
        ...
        screenshots = [
            f for f in os.listdir("validation_screenshots") if f.endswith(".png")
        ]
        screenshots.sort()
        for screenshot in screenshots[:3]:
            analyze_screenshot_detailed(
                os.path.join("validation_screenshots", screenshot)
            )
