...
def analyze_early_game_sequence(log_file: str) -> dict:
    ...
    for i, step in enumerate(steps):
        ...
        # Check start_game
        if (
            not results["goals"]["start_game"]["step_completed"]
            and game_state != "title_screen"
            and game_state != "loading"
        ):
            results["goals"]["start_game"]["step_completed"] = i

        # Check get_starter
        if not results["goals"]["get_starter"]["step_completed"] and party:
        ...
        for action, count in action_counts.items():
            if count > 15:  # Same action 75%+ of time
                results["stuck_patterns"].append(
                    {"action": action, "count": count, "percentage": (count / 20) * 100}
                )
...
def detect_blank_screen(
    screen_image: np.ndarray, white_threshold: float = 0.8, black_threshold: float = 0.8
) -> dict:
...
def capture_representative_screenshot(
    game_state: GameState, num_frames: int = 5
) -> np.ndarray | None:
    ...
    non_blank_screens = [
        (img, info)
        for img, info in zip(screenshots, blank_detections, strict=True)
        if not info["is_blank"]
    ]
    ...
    if non_blank_screens:
        ...
        best_idx = min(
            range(len(blank_detections)),
            key=lambda i: min(
                blank_detections[i]["white_percentage"],
...
def validate_state_matches_screen(
    game_state_str: str, screen_image: np.ndarray, game_info: dict
) -> dict:
    ...
    """
    validation = {"state_valid": True, "issues": [], "blank_screen": False}
    ...
    if blank_info["is_blank"]:
        ...
        validation["issues"].append(
            f"Screen is mostly {blank_info['blank_type']} ({blank_info['white_percentage']:.1%} white, {blank_info['black_percentage']:.1%} black)"
        )
    ...
    if game_state_str == "overworld":
        ...
        if not party or len(party) == 0:
            validation["issues"].append(
                "Overworld state but no party detected (should have starter)"
            )

    elif game_state_str == "dialog":
        # Dialog should have text content
        screen_text = game_info.get("screen_text", "")
        if (not screen_text or len(screen_text.strip()) < 5) and not blank_info[
            "is_blank"
        ]:
...
def analyze_game_state(game_state: GameState, game_info: dict) -> dict:
    ...
    if analysis["game_state"] == "overworld" and not analysis["has_party"]:
        ...
        analysis["progress_toward_starter"] = (
            0.1  # In overworld but no starter (problematic state)
        )
...
def run_validation_test(
    ...
    headless: bool = True,
    extend_runs: bool = True,
    max_extend_steps: int = 200,
) -> dict:
    ...
    print(
        "Target: Complete early game sequence (start_game -> get_starter -> reach_viridian)"
    )
    ...
    for run_num in range(num_runs):
        ...
        try:
            ...
            if llm_provider == "ollama":
                ...
                default_model = llm_config.get(
                    "claude_model", "claude-3-5-sonnet-20241022"
                )
                llm = ClaudeProvider(
                    api_key=api_key, model=default_model, metrics=metrics
                )
            ...
            for step in range(max_steps):
                ...
                if screen_image is not None:
                    state_validation = validate_state_matches_screen(
                        game_info.get("game_state", "unknown"), screen_image, game_info
                    )
                ...
                if (
                    ...
                    and agent.strategy
                    and "start_game" in agent.strategy.completed_goals
                    and start_game_completed_step is None
                ):
                ...
                if hasattr(agent, "strategy") and agent.strategy:
                    ...
                    agent.strategy.check_goal_completion(
                        memory_data, game_info.get("game_state", "unknown")
                    )

                    # Track first completion of each goal
                    if (
                        "start_game" in agent.strategy.completed_goals
                        and len(step_counts["start_game"]) == run_num
                    ):
                        goal_completions["start_game"] += 1
                        step_counts["start_game"].append(step)

                    if (
                        "get_starter" in agent.strategy.completed_goals
                        and len(step_counts["get_starter"]) == run_num
                    ):
                ...
                if map_id == 1 or map_id == 0x01:
                    if (
                        hasattr(agent, "strategy")
                        and agent.strategy
                        and "reach_viridian" not in agent.strategy.completed_goals
                    ):
            ...
            if not completed_early and extend_runs and final_state_analysis:
                ...
                is_in_starter_selection = final_state_analysis.get(
                    "is_in_starter_selection", False
                )
                ...
                if (
                    is_in_starter_selection or (is_in_dialog and progress > 0.2)
                ) and final_step < max_steps + max_extend_steps:
                    print(
                        f" (extending - progress: {progress:.1%})", end="", flush=True
                    )
                    for extended_steps, step in enumerate(
                        range(
                            final_step,
                            min(
                                final_step + max_extend_steps,
                                max_steps + max_extend_steps,
                            ),
                        ),
                        start=1,
                    ):
                        ...
                        if party and len(party) > 0:
                            ...
                            if any(p.get("species", 0) in starters for p in party):
                                if (
                                    ...
                                    and agent.strategy
                                    and "get_starter"
                                    not in agent.strategy.completed_goals
                                ):
                        ...
                        if (
                            extended_steps > 50
                            and final_state_analysis.get("progress_toward_starter", 0.0)
                            <= 0.3
                        ):
                            break
            ...
            if not completed_early:
                ...
                if final_state_analysis:
                    ...
                    reason_str = (
                        f" ({', '.join(failure_reason)})" if failure_reason else ""
                    )
                    print(
                        f"FAILED (timeout at step {final_step}, state: {state}{reason_str})"
                    )
                else:
        ...
        run_result = {
            ...
            "final_state": final_state_analysis
            if "final_state_analysis" in locals()
            else None,
        }
    ...
    for goal_name in goal_completions:
        results["goal_completion_rates"][goal_name] = (
            goal_completions[goal_name] / num_runs
        ) * 100
...
def print_validation_report(results: dict):
    ...
    if results["success_rate"] >= 80:
        ...
        print(
            f"[FAILURE] Below v1.0.0 requirement (need >80%, got {results['success_rate']:.1f}%)"
        )
    print()
        ...
        for run in failed_runs[:5]:  # Show first 5 failed runs
            ...
            if final_state:
                ...
                if state_validation.get("issues"):
                    print(
                        f"    - State validation issues: {', '.join(state_validation['issues'][:3])}"
                    )
    ...
    if results.get("stuck_patterns_found"):
        ...
        for pattern in results["stuck_patterns_found"]:
            print(
                f"  - {pattern['action']}: {pattern['count']} times ({pattern['percentage']:.1f}%)"
            )
        print()
...
def main():
    ...
    parser.add_argument(
        "--rom", type=str, required=True, help="Path to Pokemon Red ROM file"
    )
    parser.add_argument(
        "--runs", type=int, default=10, help="Number of test runs (default: 10)"
    )
    parser.add_argument(
        "--max-steps",
        type=int,
    ...
    parser.add_argument(
        "--display", action="store_true", help="Run with display (overrides --headless)"
    )
    parser.add_argument(
        "--no-extend",
        action="store_true",
    ...
    if args.log_file:
        ...
        # Save analysis
        output_file = (
            f"early_game_validation_{datetime.now().strftime('%Y%m%d_%H%M%S')}.json"
        )
