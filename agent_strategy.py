...
class AgentStrategy:
                ...
                if memory_data:
                    ...
                    if map_id == 0x00:  # Pallet Town
                        ...
                        if (
                            pos
                            and len(pos) >= 2
                            and self.is_at_boundary(pos, "UP", map_id)
                        ):
                            return "RIGHT"
                        return "UP"
                return "UP"
    ...
    def should_explore(
        self, stuck_count: int = 0, action_diversity: float = 1.0
    ) -> bool:
    ...
    def get_adaptive_exploration_rate(
        self, base_rate: float, stuck_count: int, action_diversity: float
    ) -> float:
    ...
    def get_strategy_context(
        self, game_state: str, memory_data: dict | None = None
    ) -> dict:
        ...
        context = {
            ...
            "exploration_mode": self.should_explore(
                0, 1.0
            ),  # Default values when called without context
        }

        # Add memory-based context if available
        if memory_data:
            context["player_position"] = memory_data.get("player_position")
            context["current_map"] = memory_data.get("current_map", {}).get(
                "map_name", "Unknown"
            )
    ...
    def check_goal_completion(
        self, memory_data: dict | None = None, game_state: str = "unknown"
    ):
        ...
        if (
            "start_game" not in self.completed_goals
            and game_state != "title_screen"
            and game_state in ["menu", "dialog", "overworld", "battle"]
        ):
        ...
        if "level_up" not in self.completed_goals:
            ...
            if party and any(p.get("level", 0) > 5 for p in party):
        ...
        if "train_pokemon" not in self.completed_goals:
            ...
            if party and any(p.get("level", 0) >= 10 for p in party):
    ...
    def get_progress_summary(self) -> dict:
        ...
        progress_percent = (
            (completed_count / total_goals * 100) if total_goals > 0 else 0
        )
