...
class GameState:
    ...
    def save_screenshot(
        self, filename: str | None = None, directory: str = "logs/screenshots"
    ) -> str:
    ...
    def get_screen_text(self) -> str:
        ...
        if self.use_enhanced_ocr and self.ocr_enhancer:
            try:
                ...
                text = self.ocr_enhancer.extract_text_enhanced(
                    screen_image, prioritize_dialog=True
                )
    ...
    def get_game_info(self) -> dict:
        ...
        if self.memory_enabled and self.memory_reader:
            ...
            should_check_memory = (
                current_step % self.memory_check_interval == 0
                or self.cached_memory_state is None
            )
        ...
        if memory_state:
                ...
                if player_pos[0] > 0 or player_pos[1] > 0:
                    ...
                    if blank_info["is_blank"]:
                        ...
                        if blank_info["blank_type"] == "white":
                            ...
                            if any(
                                word in screen_text.upper()
                                for word in ["NINTENDO", "GAME FREAK", "PRESENTS"]
                            ):
                                game_state = "title_screen"
                            elif any(
                                word in screen_text.upper()
                                for word in ["MENU", "OPTIONS", "SAVE", "NEW GAME"]
                            ):
                                game_state = "menu"
                            else:
                                game_state = (
                                    "loading"  # Likely a transition/loading screen
                                )
                        else:
            ...
            if game_state == "overworld":
                ...
                elif len(screen_text) > 0 and self.detect_text_box():
                    game_state = "dialog"
            ...
            if not screen_text or "PyBoy" in screen_text:
                ...
                word in screen_text.upper() for word in ["MENU", "OPTIONS", "SAVE"]
            ):
                game_state = "menu"
            elif any(
                word in screen_text.upper()
                for word in ["WILD", "ATTACK", "FIGHT", "RUN"]
            ):
                game_state = "battle"
            elif (
                len(screen_text) > 3
            ):  # Lowered threshold - even short garbled text might be dialogue
