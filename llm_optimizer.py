...
class ActionCache:
    ...
    def _get_key(self, screen_text: str, frame_count: int, recent_actions: list) -> str:
        ...
        state_str = (
            f"{normalized_text}|{frame_count // 100}|{','.join(recent_actions[-3:])}"
        )
        return hashlib.md5(state_str.encode()).hexdigest()

    def get(
        self, screen_text: str, frame_count: int, recent_actions: list
    ) -> str | None:
    ...
    def set(
        self, screen_text: str, frame_count: int, recent_actions: list, action: str
    ):
        """Cache an action.
...
class PromptOptimizer:
    ...
    def optimize_prompt(
        ...
        if strategy_context:
            strategy_parts = []
            if (
                "current_goal" in strategy_context
                and strategy_context["current_goal"] != "none"
            ):
                strategy_parts.append(
                    f"Goal: {strategy_context.get('goal_description', 'Unknown')}"
                )
            if "phase" in strategy_context:
                strategy_parts.append(f"Phase: {strategy_context['phase']}")
            if (
                "completed_goals" in strategy_context
                and strategy_context["completed_goals"]
            ):
                strategy_parts.append(
                    f"Completed: {', '.join(strategy_context['completed_goals'][-3:])}"
                )
        ...
        hint = PromptOptimizer._get_context_hint(
            game_state, step_count, recent_actions, strategy_context
        )

        # Action explanations
        action_explanations = PromptOptimizer._get_action_explanations(
            game_state, strategy_context, step_count
        )
    ...
    def _get_context_hint(
        ...
        if game_state == "title_screen":
            ...
            if (
                strategy_context
                and strategy_context.get("current_goal") == "reach_viridian"
            ):
                return "Exploring - move UP/NORTH to reach Route 1 and Viridian City"
            elif (
                strategy_context
                and strategy_context.get("current_goal") == "get_starter"
            ):
    ...
    @staticmethod
    def _get_action_explanations(
        game_state: str, strategy_context: dict | None = None, step_count: int = 0
    ) -> str:
        ...
        if game_state == "title_screen":
            ...
            if step_count >= 50:  # Only suggest B after character creation is done
                ...
                explanations.append(
                    "WARNING: DO NOT press B - you are creating a new character, B will cancel!"
                )
        elif game_state == "battle":
...
class RepetitionDetector:
    ...
    def check(self, action: str) -> tuple[bool, str | None]:
        ...
        if stat_pattern:
            return self._suggest_alternative(
                stat_action, f"Statistical pattern: {stat_ratio:.0%} {stat_action}"
            )
    ...
    def _suggest_alternative(
        self, current_action: str, reason: str
    ) -> tuple[bool, str | None]:
    ...
    def detect_statistical_pattern(
