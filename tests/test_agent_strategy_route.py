...
def suggest(map_id, pos, visited=()):
    ...
    return s.suggest_action_for_goal(
        goal,
        "overworld",
        {
...
class TestRoutePolicyWins:
    ...
    def test_scans_along_the_wall_when_route_direction_is_blocked(
...
class TestDoorExitManeuver:
    ...
    def _strategy_after_exit(self):
        s = AgentStrategy()
        s.update_phase(
            "overworld", {"current_map": {"map_id": 0x25}, "party": []}
        )  # inside
        s.update_phase(
            "overworld", {"current_map": {"map_id": 0x00}, "party": []}
        )  # stepped out
        return s
...
class TestEdgeScan:
    ...
    def test_unblocked_route_resumes_and_resets_scan(
        self, mock_llm_provider, mock_pyboy
    ):
        agent = self._agent(mock_llm_provider, mock_pyboy)
        agent.blocked_directions.add("UP")
