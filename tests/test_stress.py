...
class TestExtendedRuns:
    ...
    def test_memory_leak_detection(self, mock_llm_provider, mock_pyboy):
        ...
        assert growth_factor < 100, (
            f"Potential memory leak: initial {initial_size}, final {final_size}, growth {growth_factor:.2f}x"
        )
    ...
    def test_long_running_stability(self, mock_llm_provider, mock_pyboy):
        ...
        if len(step_times) > 100:
            ...
            assert late_avg < early_avg * 5, (
                f"Performance degradation: early {early_avg:.4f}s, late {late_avg:.4f}s"
            )
    ...
    def test_action_history_limit(self, mock_llm_provider, mock_pyboy):
        ...
        for _step in range(200):
            ...
            assert len(agent.action_history) <= 100, (
                f"Action history {len(agent.action_history)} exceeds limit of 100"
            )
