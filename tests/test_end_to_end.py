...
class TestEarlyGameSequence:
    ...
    def test_start_game_sequence(self, mock_llm_provider, mock_pyboy):
        ...
        assert "START" in actions_taken or "A" in actions_taken, (
            "Agent didn't attempt to start game"
        )
    ...
    def test_menu_navigation(self, mock_llm_provider, mock_pyboy):
        ...
        assert "A" in actions_taken or "B" in actions_taken, (
            "Agent didn't interact with menu"
        )
    ...
    def test_overworld_movement(self, mock_llm_provider, mock_pyboy):
        ...
        assert any(action in actions_taken for action in movement_actions), (
            "Agent didn't use movement actions"
        )
        ...
        if any(action in movement_actions for action in actions_taken[:10]):
            ...
            assert initial_position != final_position or len(set(actions_taken)) > 2, (
                "Agent didn't move or is stuck"
            )
...
class TestCompleteGameplayFlow:
    ...
    def test_dialogue_handling(self, mock_llm_provider, mock_pyboy):
        ...
        dialogue_states = [
            ...
            {
                "screen_text": "Welcome to the world",
                "frame_count": 2,
                "game_state": "dialog",
            },
        ...
        assert actions_taken.count("A") > 0, (
            "Agent didn't handle dialogue with A button"
        )
