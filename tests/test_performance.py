...
class TestPerformanceBenchmarks:
    ...
    def test_step_time_benchmark(self, mock_llm_provider, mock_pyboy):
        ...
        assert avg_step_time < 50, (
            f"Average step time {avg_step_time:.2f}ms exceeds 50ms threshold"
        )
        ...
        assert (
            stats["performance"]["step_timing"]["avg_time"] < 0.05
        )  # <50ms in seconds
    ...
    def test_llm_latency_benchmark(self, mock_llm_provider, mock_pyboy):
        ...
        if llm_stats["total_calls"] > 0:
            ...
            assert avg_latency < 500, (
                f"Average LLM latency {avg_latency:.2f}ms exceeds 500ms threshold"
            )
    ...
    def test_ocr_timing_benchmark(self, mock_pyboy):
        ...
        with patch("game_state.pytesseract") as mock_tesseract:
            ...
            if ocr_stats["total_calls"] > 0:
                ...
                assert avg_ocr_time < 500, (
                    f"Average OCR time {avg_ocr_time:.2f}ms exceeds 500ms threshold"
                )
...
class TestPerformanceRegression:
    ...
    def test_step_time_regression(self, mock_llm_provider, mock_pyboy):
        ...
        assert late_avg < early_avg * 2, (
            f"Performance degradation detected: early avg {early_avg:.4f}s, late avg {late_avg:.4f}s"
        )
    ...
    def test_memory_usage_stable(self, mock_llm_provider, mock_pyboy):
        ...
        initial_size = sys.getsizeof(agent.action_history) + sys.getsizeof(
            agent.action_cache
        )
        ...
        final_size = sys.getsizeof(agent.action_history) + sys.getsizeof(
            agent.action_cache
        )
        ...
        assert final_size < initial_size * 10, (
            f"Potential memory leak: initial {initial_size}, final {final_size}"
        )
