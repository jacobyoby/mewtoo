...
class OCREnhancer:
    ...
    def detect_text_regions(self, image: np.ndarray) -> list[dict]:
        ...
        scaled = cv2.resize(
            gray,
            (image.shape[1] * self.scale_factor, image.shape[0] * self.scale_factor),
        )
        ...
        contours, _ = cv2.findContours(
            dialog_region, cv2.RETR_EXTERNAL, cv2.CHAIN_APPROX_SIMPLE
        )
        ...
        menu_contours, _ = cv2.findContours(
            menu_region, cv2.RETR_EXTERNAL, cv2.CHAIN_APPROX_SIMPLE
        )
    ...
    def extract_text_from_region(
        self, image: np.ndarray, region: dict, use_character_level: bool = False
    ) -> str:
        ...
        # Convert to grayscale
        gray = (
            cv2.cvtColor(scaled, cv2.COLOR_RGB2GRAY)
    ...
    def extract_text_enhanced(
        self, image: np.ndarray, prioritize_dialog: bool = True
    ) -> str:
        ...
        if prioritize_dialog:
            regions.sort(
                key=lambda r: (r["type"] == TextRegion.DIALOG_BOX, r["confidence"]),
                reverse=True,
            )
